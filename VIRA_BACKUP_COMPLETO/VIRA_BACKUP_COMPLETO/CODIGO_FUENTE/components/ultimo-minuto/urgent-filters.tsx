...
interface UrgentFiltersProps {
  ...
  isLoading: boolean
  newsCount: number
  fuentes?: any[] // Fuentes de Supabase
}
...
export function UrgentFilters({
  ...
  // Generar opciones de región desde las fuentes de Supabase
  const getRegionOptions = () => {
    if (!fuentes || fuentes.length === 0) {
      // Si no hay datos de Supabase, usar opciones predefinidas como fallback
      return defaultRegionOptions;
    }
    
    // Obtener regiones únicas de las fuentes de Supabase (campo 'nombre' contiene la región)
    const uniqueRegions = Array.from(new Set(fuentes.map(f => f.nombre).filter(Boolean)));
    
    // Mapear regiones con iconos apropiados
    ...
    // Crear opciones con la opción "Todas" al inicio, seguida de las regiones de Supabase
    const regionOptions = [
      { value: 'all', label: 'Todas las regiones', flag: '🇨🇱' },
    ...
    // Si no hay regiones de Supabase, agregar las predefinidas como respaldo
