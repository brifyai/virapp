...
export default function IntegracionesPage() {
  ...
  const handleSavePodcastConfig = async () => {
    ...
    if (!hasValidConfig) {
      alert('Por favor configura al menos una plataforma de podcast')
      return
    }
    ...
    try {
      ...
      alert(`Configuración guardada para: ${enabledPlatforms}. Los noticieros se subirán automáticamente a estas plataformas.`)
      setIsPodcastModalOpen(false)
      
    } catch (error) {
      alert('Error al guardar la configuración de podcasts')
    } finally {
  ...
  const handleSaveScrapingConfig = async () => {
    if (scrapingConfig.sites.length === 0) {
      alert('Por favor configura al menos un sitio web para scraping')
      return
    }
    ...
    try {
      ...
      alert(`Configuración de scraping guardada para: ${enabledSites}. VIRA utilizará ${scrapingConfig.engine} para extraer noticias automáticamente.`)
      setIsScrapingModalOpen(false)
      
    } catch (error) {
      alert('Error al guardar la configuración de scraping')
    } finally {
      setIsConnecting(false)
    }
  }
  ...
  const handleAddScrapingSite = () => {
    if (!newSite.name.trim() || !newSite.url.trim()) {
      alert('Por favor complete nombre y URL del sitio')
      return
    }
  ...
  const handleTestSelector = async (url: string, selector: string) => {
    if (!url.trim() || !selector.trim()) {
      alert('Por favor proporciona URL y selector para probar')
      return
    }
    
    // Simular test de selector
    alert(`Probando selector "${selector}" en ${url}...\n\nResultado: ✅ Selector válido - Se encontraron 5 elementos`)
  }
  ...
  const handleSaveAiConfig = async () => {
    if (!aiConfig.abacus.enabled && !aiConfig.groq.enabled) {
      alert('Por favor habilita al menos una API de IA')
      return
    }

    if (aiConfig.groq.enabled && !aiConfig.groq.apiKey.trim()) {
      alert('Por favor configura la API Key de Groq')
      return
    }
    ...
    try {
      ...
      alert(`Configuración de IA guardada para: ${enabledApis.join(', ')}. VIRA procesará automáticamente las noticias scrapeadas usando estos modelos.`)
  ...
  const handleTestAiPrompt = async (promptType: 'rewrite' | 'humanize' | 'adapt') => {
    ...
    if (aiConfig.groq.enabled && aiConfig.groq.apiKey.trim()) {
      ...
      alert('❌ Por favor habilita al menos una API de IA (Abacus AI o Groq)')
      return
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(`✅ Test de ${promptType} exitoso con ${providerName} (${data.model})!\n\nNoticia original:\n"${sampleNews}"\n\nNoticia procesada:\n"${data.processedContent}"`)
      } else {
        alert(`❌ Error en el test: ${data.error}`)
      }
    } catch (error) {
  ...
  const handleSaveSocialConfig = async () => {
    ...
    if (!hasValidConfig) {
      alert('Por favor configura al menos una red social')
      return
    }
    ...
    try {
      ...
      alert(`Configuración guardada para: ${enabledPlatforms}. Los noticieros se publicarán automáticamente en estas plataformas.`)
      setIsSocialModalOpen(false)
      
    } catch (error) {
      alert('Error al guardar la configuración de redes sociales')
    } finally {
      setIsConnecting(false)
    }
  }
  ...
  const handleConfigureRadioSoftware = async () => {
    if (!selectedSoftware) {
      alert('Por favor selecciona un software de automatización')
      return
    }
    ...
    try {
      ...
      alert(`Configuración guardada para ${software?.name}. Los noticieros se enviarán automáticamente a tu sistema.`)
      ...
      alert('Error al configurar la conexión')
    } finally {
      setIsConnecting(false)
    }
  }
